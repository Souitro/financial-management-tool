...
<title>Reports | <?= htmlspecialchars($user['company_name']) ?></title>
...
<aside id="sidebar">
  ...
  <nav class="sb-nav">
    ...
    <div class="sb-item <?= !$isManager ? 'locked' : '' ?> active" data-panel="reports" onclick="window.location.href='report.php'">
...
<div class="panel active" id="panel-reports">
  <div class="pg-hero rv">
    <h1>Financial <em>Reports</em></h1>
    <p>Full financials · <a href="reports.php" style="color:var(--s)">Open full reports page →</a></p>
  </div>
  <?php if(!$isManager): ?>
  <div style="text-align:center;color:var(--text3);padding:60px 0;font-size:14px">🔒 Reports are restricted to Manager and CEO roles.</div>
  <?php else: ?>
  <div class="g4 rv">
    <div class="gc" data-c style="--cc:var(--green)">
      <div class="gc-t"><span class="dot"></span>Month Revenue</div>
      <div class="sv"><?= fmtRK($monthly_revenue) ?></div>
      <div class="sl"><?= date('F Y') ?></div>
      <div class="sd <?= $revenue_delta >= 0 ? 'up' : 'dn' ?>"><?= $revenue_delta >= 0 ? '↑' : '↓' ?> <?= abs($revenue_delta) ?>% vs last month</div>
    </div>
    <div class="gc" data-c style="--cc:var(--yellow)">
      ...
      <div class="sd <?= $overdue_count>0?'dn':'up' ?>"><?= $overdue_count ?> overdue</div>
    </div>
    <div class="gc" data-c style="--cc:var(--s)">
      <div class="gc-t"><span class="dot"></span>Total Clients</div>
      <div class="sv"><?= $total_clients ?></div>
      <div class="sl">Active this company</div>
    </div>
    <div class="gc" data-c style="--cc:var(--purple)">
      <div class="gc-t"><span class="dot"></span>Invoices (Month)</div>
      <div class="sv"><?= $monthly_invoice_count ?></div>
      <div class="sl"><?= date('F Y') ?></div>
    </div>
  </div>
  <div style="text-align:center;margin-top:12px">
    <a href="reports.php" class="btn btn-p" style="text-decoration:none">Open Full Reports →</a>
  </div>
  <?php endif; ?>
</div>
