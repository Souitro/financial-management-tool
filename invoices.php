...
if (
    ...
    if ($invoice_id > 0 && in_array($new_status, $allowed, true)) {
        try {
            $pdo->prepare("
                UPDATE invoices SET status=?, updated_at=NOW()
                WHERE id=? AND company_id=?
            ")->execute([$new_status, $invoice_id, $current_company_id]);

            $pdo->prepare("
                INSERT INTO audit_log (user_id, company_id, action, target, ip_address, user_agent)
                VALUES (?, ?, 'INVOICE_STATUS_CHANGED', ?, ?, ?)
            ")->execute([
                $current_user_id,
                $current_company_id,
                'invoices:' . $invoice_id,
...
<title>Invoices | <?= htmlspecialchars($user['company_name']) ?></title>
...
<aside id="sidebar">
  ...
  <nav class="sb-nav">
    ...
    <div class="sb-item active" data-panel="invoices" onclick="window.location.href='invoices.php'">
...
<!-- ════════════ PANEL: INVOICES ════════════ -->
<div class="panel active" id="panel-invoices">
  <div class="pg-hero rv" style="display:flex;justify-content:space-between;align-items:flex-start">
    <div>
      <h1>Invoices</h1>
      <p><?= count($recent_invoices) ?> invoice(s) · <?= htmlspecialchars($user['company_name']) ?></p>
    </div>
    <a href="invoice-generate.php" class="btn btn-p" style="text-decoration:none">+ New Invoice</a>
  </div>

  <!-- Status filter tabs -->
  <div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap" id="filter-tabs">
    <button class="btn btn-g btn-sm filter-tab active" data-filter="all" onclick="filterInvoices('all')">All</button>
    <button class="btn btn-g btn-sm filter-tab" data-filter="draft" onclick="filterInvoices('draft')">Draft</button>
    <button class="btn btn-g btn-sm filter-tab" data-filter="sent" onclick="filterInvoices('sent')">Sent</button>
    <button class="btn btn-g btn-sm filter-tab" data-filter="paid" onclick="filterInvoices('paid')">Paid</button>
    <button class="btn btn-g btn-sm filter-tab" data-filter="overdue" onclick="filterInvoices('overdue')">Overdue</button>
    <button class="btn btn-g btn-sm filter-tab" data-filter="cancelled" onclick="filterInvoices('cancelled')">Cancelled</button>
  </div>

  <div class="gc rv" data-c>
    <div class="gc-t"><span class="dot"></span>Invoice List</div>
    <?php if(empty($recent_invoices)): ?>
    <div style="text-align:center;color:var(--text3);padding:40px 0;font-size:13px">
      No invoices yet. <a href="invoice-generate.php" style="color:var(--s)">Create your first invoice →</a>
    </div>
    <?php else: ?>
    <table class="tbl" id="invoices-table">
      ...
      <tbody>
        ...
        <tr data-status="<?= htmlspecialchars($inv['status']) ?>">
          ...
          <td style="color:var(--text2);font-size:12px"><?= date('d M Y', strtotime($inv['issue_date'])) ?></td>
          <td style="color:<?= strtotime($inv['due_date']) < time() && $inv['status'] !== 'paid' ? 'var(--a)' : 'var(--text2)' ?>;font-size:12px"><?= date('d M Y', strtotime($inv['due_date'])) ?></td>
          <td class="mono"><?= fmtR((float)$inv['total_amount']) ?></td>
          <td><?= statusTag($inv['status']) ?></td>
          <td>
            <select class="li btn-sm" style="width:auto;padding:5px 10px;font-size:12px" onchange="updateStatus(<?= $inv['id'] ?>, this.value, this)">
...
function filterInvoices(status) {
    ...
    document.querySelectorAll('.filter-tab').forEach(tab => {
        ...
        tab.style.borderColor = tab.dataset.filter === status ? 'var(--s)' : '';
        tab.style.color       = tab.dataset.filter === status ? 'var(--s)' : '';
    });
}

// Update invoice status via AJAX
async function updateStatus(invoiceId, newStatus, selectEl) {
    const res = await fetch('invoices.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'X-Requested-With': 'XMLHttpRequest'
        },
        body: new URLSearchParams({ invoice_id: invoiceId, status: newStatus })
    }).then(r => r.json()).catch(() => ({ ok: false }));

    if (res.ok) {
        // Update the row's data-status so filter still works
        selectEl.closest('tr').dataset.status = newStatus;
        // Update the status tag cell
        const statusCell = selectEl.closest('tr').querySelector('td:nth-child(6)');
        const tagMap = {
            paid:      ['tg','Paid'],
            sent:      ['ty','Sent'],
            overdue:   ['tr','Overdue'],
        ...
        const [cls, lbl] = tagMap[newStatus] || ['tb', newStatus];
        statusCell.innerHTML = `<span class="tag ${cls}">${lbl}</span>`;
        toast('Status updated to ' + newStatus, 'ts');
    } else {
        toast('Failed to update status', 'te');
        // Revert select
        selectEl.value = selectEl.dataset.original;
    }
}

// Store original value on focus so we can revert on failure
document.querySelectorAll('#invoices-table select').forEach(sel => {
