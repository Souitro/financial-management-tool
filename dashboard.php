...
<div class="panel active" id="panel-overview">
  <div class="pg-hero rv">
    <h1>Good <?= (date('H') < 12 ? 'morning' : (date('H') < 17 ? 'afternoon' : 'evening')) ?>, <em><?= htmlspecialchars(explode(' ', $user['name'])[0]) ?></em></h1>
    <p><?= htmlspecialchars($user['company_name']) ?> · <?= date('l, d F Y') ?> · <?= htmlspecialchars($user['role']) ?></p>
  </div>

  <!-- KPI row -->
  <div class="g4">
    <div class="gc rv" data-c style="--cc:var(--green)">
      <div class="gc-t"><span class="dot"></span>Monthly Revenue</div>
      <div class="sv"><?= fmtRK($monthly_revenue) ?></div>
      <div class="sl">Paid invoices · <?= date('F Y') ?></div>
      <div class="sd <?= $revenue_delta >= 0 ? 'up' : 'dn' ?>">
        <?= $revenue_delta >= 0 ? '↑' : '↓' ?> <?= abs($revenue_delta) ?>% vs last month
      </div>
    </div>
    <div class="gc rv" data-c style="--cc:var(--yellow)">
      ...
      <div class="sd <?= $overdue_count > 0 ? 'dn' : 'up' ?>">
        <?= $overdue_count > 0 ? "⚠ {$overdue_count} overdue" : '✓ All on track' ?>
      </div>
    </div>
    <div class="gc rv" data-c style="--cc:var(--a)">
      <div class="gc-t"><span class="dot"></span>Low Stock</div>
      <div class="sv"><?= $low_stock_warnings ?></div>
      <div class="sl">Items below reorder level</div>
      <div class="sd <?= $low_stock_warnings > 0 ? 'wn' : 'up' ?>">
        <?= $low_stock_warnings > 0 ? '⚠ Restock needed' : '✓ Stock healthy' ?>
      </div>
    </div>
    <div class="gc rv" data-c style="--cc:var(--s)">
      <div class="gc-t"><span class="dot"></span>Active Clients</div>
      <div class="sv"><?= $total_clients ?></div>
      <div class="sl">Registered this company</div>
      <div class="sd up">↑ <?= $monthly_invoice_count ?> invoices this month</div>
    </div>
  </div>

  <!-- Revenue chart + Invoice status -->
  <div class="g2">
    <div class="gc rv" data-c>
      <div class="gc-t"><span class="dot"></span>Revenue · Last 6 Months</div>
      <?php
      $maxRev = max(array_column($chart_data, 'revenue') ?: [1]);
      ?>
      <div class="chart-bars" id="rev-chart">
        <?php foreach($chart_data as $cd):
          $h = $maxRev > 0 ? round(($cd['revenue']/$maxRev)*100) : 4;
          $opacity = 0.35 + ($h/100)*0.65;
        ?>
        <div class="chart-bar-wrap">
          <div class="chart-bar-val"><?= fmtRK((float)$cd['revenue']) ?></div>
          <div class="chart-bar" style="height:<?= $h ?>%;background:rgba(14,111,203,<?= $opacity ?>)"></div>
          <div class="chart-bar-lbl"><?= htmlspecialchars($cd['mo']) ?></div>
        </div>
        <?php endforeach; ?>
        <?php if(empty($chart_data)): ?>
        <div style="width:100%;display:flex;align-items:center;justify-content:center;color:var(--text3);font-size:13px">No data yet</div>
        <?php endif; ?>
      </div>
    </div>
    <div class="gc rv" data-c style="--cc:var(--purple)">
      <div class="gc-t"><span class="dot"></span>Invoice Status Breakdown</div>
      <?php
      $statuses = ['paid'=>['var(--green)','Paid'],'sent'=>['var(--yellow)','Sent'],'overdue'=>['var(--a)','Overdue'],'draft'=>['#5aabee','Draft']];
      $totalInv = array_sum(array_column($inv_status, 'total')) ?: 1;
      foreach($statuses as $key=>[$color,$label]):
        $rec = $inv_status[$key] ?? ['cnt'=>0,'total'=>0];
        $pct = round(($rec['total']/$totalInv)*100);
      ?>
      <div style="margin-bottom:11px">
        <div style="display:flex;justify-content:space-between;font-size:12.5px;margin-bottom:4px">
          <span style="color:<?= $color ?>"><?= $label ?> (<?= $rec['cnt'] ?>)</span>
          <span class="mono"><?= fmtRK((float)$rec['total']) ?></span>
        </div>
        <div class="prog"><div class="prog-f" style="width:<?= $pct ?>%;background:<?= $color ?>"></div></div>
      </div>
      <?php endforeach; ?>
      <div style="border-top:1px solid var(--border);margin-top:14px;padding-top:12px;display:flex;justify-content:space-between">
        <div><div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:2px">Total Invoiced</div>
        <div class="mono" style="font-size:16px"><?= fmtRK($totalInv) ?></div></div>
        <div style="text-align:right"><div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:.07em;margin-bottom:2px">Collected</div>
        <div class="mono" style="font-size:16px;color:var(--green)"><?= fmtRK((float)($inv_status['paid']['total'] ?? 0)) ?></div></div>
      </div>
    </div>
  </div>
  ...
  <div class="g2">
    <div class="gc rv" data-c style="--cc:var(--yellow)">
      <div class="gc-t" style="justify-content:space-between">
        <div style="display:flex;align-items:center;gap:8px"><span class="dot"></span>Recent Invoices</div>
        <a href="invoice-generate.php" class="btn btn-g btn-sm" style="text-decoration:none">+ New</a>
      </div>
      <?php if(empty($recent_invoices)): ?>
      <div style="text-align:center;color:var(--text3);padding:24px 0;font-size:13px">No invoices yet</div>
      <?php else: ?>
      <table class="tbl">
        ...
        <tbody>
          ...
          <tr>
            <td class="mono" style="font-size:12px"><?= htmlspecialchars($inv['invoice_number']) ?></td>
            <td><?= htmlspecialchars($inv['client_name']) ?></td>
            <td class="mono"><?= fmtRK((float)$inv['total_amount']) ?></td>
    ...
    <div class="gc rv" data-c style="--cc:var(--s)">
      ...
      <div style="text-align:center;color:var(--text3);padding:24px 0;font-size:13px">No clients yet</div>
      <?php else: ?>
      <?php
      $maxClientTotal = max(array_column($top_clients,'total') ?: [1]);
      foreach($top_clients as $cl):
        $pct = $maxClientTotal > 0 ? round(($cl['total']/$maxClientTotal)*100) : 0;
      ?>
      <div style="margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;font-size:12.5px;margin-bottom:4px">
          <span><?= htmlspecialchars($cl['name']) ?></span>
          <span class="mono"><?= fmtRK((float)$cl['total']) ?></span>
        </div>
        <div class="prog"><div class="prog-f" style="width:<?= $pct ?>%;background:var(--s)"></div></div>
      </div>
      <?php endforeach; ?>
      <?php endif; ?>
    </div>
  ...
  <div class="g2">
    <div class="gc rv" data-c style="--cc:var(--green)">
      <div class="gc-t"><span class="dot"></span>Recent Payments</div>
      <?php if(empty($recent_payments)): ?>
      <div style="text-align:center;color:var(--text3);padding:24px 0;font-size:13px">No payments recorded yet</div>
      <?php else: ?>
      <table class="tbl">
        ...
        <tbody>
          ...
          <tr>
            <td><?= htmlspecialchars($pay['client_name']) ?><br/><span style="font-size:11px;color:var(--text3)"><?= htmlspecialchars($pay['invoice_number']) ?></span></td>
            <td class="mono" style="color:var(--green)"><?= fmtRK((float)$pay['amount']) ?></td>
            <td><span class="tag tt"><?= htmlspecialchars(strtoupper($pay['method'])) ?></span></td>
            <td style="font-size:12px;color:var(--text2)"><?= date('d M', strtotime($pay['payment_date'])) ?></td>
          </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
    ...
    <div class="gc rv" data-c style="--cc:var(--purple)">
      <div class="gc-t"><span class="dot"></span>Recent Activity</div>
      <?php if(empty($recent_logs)): ?>
      <div style="text-align:center;color:var(--text3);padding:24px 0;font-size:13px">No activity yet</div>
      <?php else: ?>
      <?php
      $actionIcons = ['LOGIN_SUCCESS'=>'✓','LOGIN_FAIL'=>'✕','INVOICE_SENT'=>'📄','INVOICE_DRAFT_SAVED'=>'💾','PASSWORD_SET'=>'🔐','PROFILE_UPDATE'=>'✏️','PASSWORD_CHANGE'=>'🔑'];
      $actionColors= ['LOGIN_SUCCESS'=>'rgba(26,220,142,.1)','LOGIN_FAIL'=>'rgba(255,94,58,.1)','INVOICE_SENT'=>'rgba(14,111,203,.1)','INVOICE_DRAFT_SAVED'=>'rgba(0,200,200,.1)','PASSWORD_SET'=>'rgba(168,85,247,.1)','PROFILE_UPDATE'=>'rgba(240,180,41,.1)','PASSWORD_CHANGE'=>'rgba(168,85,247,.1)'];
      foreach($recent_logs as $log):
        $ico = $actionIcons[$log['action']] ?? '●';
        $bg  = $actionColors[$log['action']] ?? 'rgba(255,255,255,.06)';
      ?>
      <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,.04)">
        <div style="width:30px;height:30px;border-radius:8px;background:<?= $bg ?>;display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0"><?= $ico ?></div>
        <div style="flex:1;min-width:0">
          <div style="font-size:12.5px;font-weight:600"><?= htmlspecialchars($log['action']) ?></div>
          <div style="font-size:11px;color:var(--text2)"><?= htmlspecialchars($log['target']) ?></div>
        </div>
        <div style="font-size:10.5px;color:var(--text3);flex-shrink:0"><?= date('d M H:i', strtotime($log['created_at'])) ?></div>
      </div>
      <?php endforeach; ?>
      <?php endif; ?>
    </div>
