        ...
        try {
            $pdo->prepare("
                INSERT INTO inventory
                    (company_id, created_by, sku, name, description, category,
                     unit_price, cost_price, quantity_on_hand, reorder_level)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ")->execute([
                $current_company_id,
                $current_user_id,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit_item') {
    ...
    if ($item_id > 0 && !empty($name) && !empty($sku)) {
        try {
            $pdo->prepare("
                UPDATE inventory
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_item') {
    ...
    if ($item_id > 0) {
        try {
            // Soft delete — preserves invoice line item history
            $pdo->prepare("
                UPDATE inventory SET is_active=0
                WHERE id=? AND company_id=?
            ")->execute([$item_id, $current_company_id]);
        } catch(\PDOException $e) {}
    }
    header('Location: inventory.php');
    exit;
...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT id, name, sku, category, description, unit_price, cost_price, quantity_on_hand, reorder_level, is_active
FROM inventory WHERE company_id=? ORDER BY name ASC
    ");
    $stmt->execute([$current_company_id]);
...
<title>Inventory | <?= htmlspecialchars($user['company_name']) ?></title>
...
<aside id="sidebar">
  ...
  <nav class="sb-nav">
    ...
    <div class="sb-item active" data-panel="inventory" onclick="window.location.href='inventory.php'">
...
<div class="panel active" id="panel-inventory">
  <div class="pg-hero rv" style="display:flex;justify-content:space-between;align-items:flex-start">
    <div>
      <h1>Inventory</h1>
      <p>Stock management · <?= htmlspecialchars($user['company_name']) ?></p>
    </div>
    <button class="btn btn-p" onclick="document.getElementById('modal-add-item').classList.add('open')">+ Add Item</button>
  </div>

  <?php if($low_stock_warnings > 0): ?>
  <div style="background:rgba(255,94,58,.08);border:1px solid rgba(255,94,58,.25);border-radius:12px;padding:13px 16px;margin-bottom:16px;font-size:13px;color:var(--a)">
    ⚠ <?= $low_stock_warnings ?> item(s) are at or below reorder level — restock soon.
  </div>
  <?php endif; ?>

  <div class="gc rv" data-c style="--cc:var(--a)">
    <div class="gc-t"><span class="dot"></span>All Inventory Items</div>
    <div style="overflow-x:auto;overflow-y:auto;max-height:520px;">
    <?php if(empty($inventory_items)): ?>
    <div style="text-align:center;color:var(--text3);padding:40px 0;font-size:13px">No inventory items yet.</div>
    <?php else: ?>
    <table class="tbl">
      <thead><tr><th>Item</th><th>SKU</th><th>Category</th><th>Unit Price</th><th>Cost Price</th><th>Stock</th><th>Reorder At</th><th>Status</th><th>Actions</th></tr></thead>
      <tbody>
        ...
        <tr>
          ...
          <td class="mono" style="font-size:11.5px;color:var(--text2)"><?= htmlspecialchars($item['sku']) ?></td>
          <td><span class="tag tb"><?= htmlspecialchars($item['category'] ?? '—') ?></span></td>
          <td class="mono"><?= fmtR((float)$item['unit_price']) ?></td>
          <td class="mono" style="color:var(--text2)"><?= fmtR((float)$item['cost_price']) ?></td>
          <td class="mono" style="color:<?= $isLow ? 'var(--a)' : 'var(--text)' ?>"><?= $item['quantity_on_hand'] ?></td>
          <td class="mono" style="color:var(--text2)"><?= $item['reorder_level'] ?></td>
          <td><?= $isLow ? "<span class='tag tr'>Low Stock</span>" : "<span class='tag tg'>OK</span>" ?></td>
          <td style="display:flex;gap:6px">
            <button class="btn btn-g btn-sm" onclick="openEditItem(
              <?= $item['id'] ?>,
              '<?= addslashes(htmlspecialchars($item['name'])) ?>',
              '<?= addslashes(htmlspecialchars($item['sku'])) ?>',
              '<?= addslashes(htmlspecialchars($item['category'] ?? '')) ?>',
            ...
            <button class="btn btn-sm" style="background:rgba(255,94,58,.1);border:1px solid rgba(255,94,58,.2);color:var(--a)" onclick="deleteItem(<?= $item['id'] ?>, '<?= addslashes(htmlspecialchars($item['name'])) ?>')">Delete</button>
          </td>
        </tr>
        <?php endforeach; ?>
      </tbody>
...
<div class="modal-bg" id="modal-add-item">
  <div class="modal-box">
    ...
    <form method="POST" action="inventory.php">
      ...
      <div class="fg2">
        <div class="fw"><label class="f-lbl">Item Name</label><input class="li" name="name" required placeholder="Product or service name"/></div>
        <div class="fw"><label class="f-lbl">SKU</label><input class="li" name="sku" required placeholder="Unique stock code"/></div>
      </div>
      <div class="fg2">
        <div class="fw"><label class="f-lbl">Category</label><input class="li" name="category" placeholder="e.g. Hardware, Software"/></div>
        <div class="fw"><label class="f-lbl">Unit Price (R)</label><input class="li" type="number" name="unit_price" step="0.01" min="0" required placeholder="0.00"/></div>
      </div>
      <div class="fg2">
        <div class="fw"><label class="f-lbl">Cost Price (R)</label><input class="li" type="number" name="cost_price" step="0.01" min="0" placeholder="0.00"/></div>
        <div class="fw"><label class="f-lbl">Quantity on Hand</label><input class="li" type="number" name="quantity_on_hand" min="0" placeholder="0"/></div>
      </div>
      <div class="fw"><label class="f-lbl">Reorder Level</label><input class="li" type="number" name="reorder_level" min="0" value="10"/></div>
      <div class="fw"><label class="f-lbl">Description (optional)</label><textarea class="li" name="description" rows="2" placeholder="Brief description…"></textarea></div>
      <div class="mf">
        <button type="button" class="btn btn-g" onclick="document.getElementById('modal-add-item').classList.remove('open')">Cancel</button>
        <button type="submit" class="btn btn-p">Add Item</button>
      </div>
    </form>
  </div>
</div>
...
<!-- Edit Item Modal -->
<div class="modal-bg" id="modal-edit-item">
  <div class="modal-box">
    ...
    <form method="POST" action="inventory.php">
      ...
      <div class="fg2">
        <div class="fw"><label class="f-lbl">Item Name</label><input class="li" name="name" id="edit-item-name" required/></div>
        <div class="fw"><label class="f-lbl">SKU</label><input class="li" name="sku" id="edit-item-sku" required/></div>
      </div>
      <div class="fg2">
        <div class="fw"><label class="f-lbl">Category</label><input class="li" name="category" id="edit-item-category"/></div>
        <div class="fw"><label class="f-lbl">Unit Price (R)</label><input class="li" type="number" name="unit_price" id="edit-item-price" step="0.01" min="0"/></div>
      </div>
      <div class="fg2">
        <div class="fw"><label class="f-lbl">Cost Price (R)</label><input class="li" type="number" name="cost_price" id="edit-item-cost" step="0.01" min="0"/></div>
        <div class="fw"><label class="f-lbl">Quantity on Hand</label><input class="li" type="number" name="quantity_on_hand" id="edit-item-qty" min="0"/></div>
      </div>
      <div class="fw"><label class="f-lbl">Reorder Level</label><input class="li" type="number" name="reorder_level" id="edit-item-reorder" min="0"/></div>
      <div class="fw"><label class="f-lbl">Description (optional)</label><textarea class="li" name="description" id="edit-item-desc" rows="2"></textarea></div>
      <div class="mf">
        <button type="button" class="btn btn-g" onclick="document.getElementById('modal-edit-item').classList.remove('open')">Cancel</button>
...
<form method="POST" action="inventory.php" id="form-delete-item">
  <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>"/>
...
function openEditItem(id, name, sku, category, description, unit_price, cost_price, qty, reorder) {
    ...
    document.getElementById('edit-item-desc').value     = description;
    document.getElementById('edit-item-price').value    = unit_price;
    document.getElementById('edit-item-cost').value     = cost_price;
...
function deleteItem(id, name) {
    if (confirm('Delete "' + name + '" from inventory?\n\nExisting invoice lines will be preserved.')) {
        document.getElementById('delete-item-id').value = id;
        document.getElementById('form-delete-item').submit();
    }
}
