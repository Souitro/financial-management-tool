...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_client') {
    ...
    if (!empty($name)) {
        $pdo->prepare("
            INSERT INTO clients (company_id, name, email, phone, address, vat_number)
            VALUES (?, ?, ?, ?, ?, ?)
        ")->execute([
            $current_company_id,
            $name,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit_client') {
    ...
    if ($client_id > 0 && !empty($name)) {
        $pdo->prepare("
            UPDATE clients
            SET name=?, email=?, phone=?, address=?, vat_number=?
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_client') {
    ...
    if ($client_id > 0) {
        ...
        $pdo->prepare("
            ...
            WHERE id=? AND company_id=?
        ")->execute([$client_id, $current_company_id]);
    }
    header('Location: clients.php');
    exit;
}
...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT cl.id, cl.name, cl.email, cl.phone, cl.address, cl.vat_number,
...
<title>Clients | <?= htmlspecialchars($user['company_name']) ?></title>
...
<div class="panel active" id="panel-clients">
  <div class="pg-hero rv">
    <h1>Clients</h1>
    <p><?= $total_clients ?> active client(s) · <?= htmlspecialchars($user['company_name']) ?></p>
  </div>
  <div class="gc rv" data-c style="--cc:var(--s)">
    
    <div class="gc-t" style="justify-content:space-between">
      <div style="display:flex;align-items:center;gap:8px"><span class="dot"></span>Client Directory
    </div>
      <a href="clients.php?action=new" class="btn btn-p btn-sm" style="text-decoration:none">+ New Client</a>
    </div>
    <?php if(empty($clients_list)): ?>
    <div style="text-align:center;color:var(--text3);padding:40px 0;font-size:13px">No clients yet.</div>
    <?php else: ?>
    <table class="tbl">
      <thead><tr><th>Name</th><th>Email</th><th>Phone</th><th>Invoices</th><th>Lifetime Value</th><th>Actions</th></tr></thead>
      <tbody>
        <?php foreach($clients_list as $cl): ?>
        <tr>
          <td style="font-weight:500"><?= htmlspecialchars($cl['name']) ?></td>
          <td style="color:var(--text2);font-size:12.5px"><?= htmlspecialchars($cl['email'] ?? '—') ?></td>
          <td style="color:var(--text2);font-size:12.5px"><?= htmlspecialchars($cl['phone'] ?? '—') ?></td>
          <td class="mono"><?= $cl['invoice_count'] ?></td>
          <td class="mono" style="color:var(--green)"><?= fmtRK((float)$cl['lifetime_value']) ?></td>
          <td>
          <button class="btn btn-g btn-sm" onclick="openEdit(<?= $cl['id'] ?>, '<?= addslashes(htmlspecialchars($cl['name'])) ?>', '<?= addslashes(htmlspecialchars($cl['email'] ?? '')) ?>', '<?= addslashes(htmlspecialchars($cl['phone'] ?? '')) ?>', '<?= addslashes(htmlspecialchars($cl['address'] ?? '')) ?>', '<?= addslashes(htmlspecialchars($cl['vat_number'] ?? '')) ?>')">Edit</button>
          <button class="btn btn-sm" style="background:rgba(255,94,58,.1);border:1px solid rgba(255,94,58,.2);color:var(--a)" onclick="deleteClient(<?= $cl['id'] ?>, '<?= addslashes(htmlspecialchars($cl['name'])) ?>')">Delete</button>
          </td>
        </tr>
        <?php endforeach; ?>
      </tbody>
...
<div class="modal-bg" id="modal-add-client">
  <div class="modal-box">
    ...
    <form method="POST" action="clients.php">
      ...
      <div class="fw"><label class="f-lbl">Full Name / Company</label><input class="li" name="name" required placeholder="Client or company name"/></div>
      <div class="fw"><label class="f-lbl">Email</label><input class="li" type="email" name="email" placeholder="client@example.co.za"/></div>
      <div class="fw"><label class="f-lbl">Phone</label><input class="li" name="phone" placeholder="+27 82 000 0000"/></div>
      <div class="fw"><label class="f-lbl">Address</label><textarea class="li" name="address" rows="2" placeholder="Street, City, Province"></textarea></div>
      <div class="fw"><label class="f-lbl">VAT Number (optional)</label><input class="li" name="vat_number" placeholder="Leave blank if not applicable"/></div>
      <div class="mf">
        <button type="button" class="btn btn-g" onclick="document.getElementById('modal-add-client').classList.remove('open')">Cancel</button>
        <button type="submit" class="btn btn-p">Save Client</button>
      </div>
    </form>
  </div>
</div>
...
<!-- Edit Client Modal -->
<div class="modal-bg" id="modal-edit-client">
  <div class="modal-box">
    <h3>Edit Client</h3>
    <form method="POST" action="clients.php">
      <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>"/>
      ...
      <div class="fw"><label class="f-lbl">Full Name / Company</label><input class="li" name="name" id="edit-name" required/></div>
      <div class="fw"><label class="f-lbl">Email</label><input class="li" type="email" name="email" id="edit-email"/></div>
      <div class="fw"><label class="f-lbl">Phone</label><input class="li" name="phone" id="edit-phone"/></div>
      <div class="fw"><label class="f-lbl">Address</label><textarea class="li" name="address" id="edit-address" rows="2"></textarea></div>
      <div class="fw"><label class="f-lbl">VAT Number (optional)</label><input class="li" name="vat_number" id="edit-vat"/></div>
      <div class="mf">
        <button type="button" class="btn btn-g" onclick="document.getElementById('modal-edit-client').classList.remove('open')">Cancel</button>
...
<!-- Delete Client Form (hidden, submitted by JS) -->
<form method="POST" action="clients.php" id="form-delete-client">
  <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>"/>
...
function openEdit(id, name, email, phone, address, vat) {
